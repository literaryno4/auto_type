...
for types in range(num_type):
    ...
    ans_style = input("请输入答案页码和标号风格（例如：TP or P）：")
    ans_per_page = (num_question // (ans_end_page - ans_page + 1))

    ## begin write content
    for i in range(num_question):
        f.write("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n")
        f.write("题号：" + str(bango) + next_line)
        f.write("题型：解答" + next_line)
        f.write("题目页码：" + ques_style + str(i+1) + "(" + str(types) + ")" + next_line)
        ...
        f.write("解答页码：" + ans_style  +str(ans_page + i // ans_per_page) + "," + "Q" + str(i+1) + "(" + str(types) + ")" + next_line)
